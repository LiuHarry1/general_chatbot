              ...
              <div className="px-4 pt-4 pb-2">
                ...
                <div className="space-y-1">
                  {attachments.map((attachment, index) => (
                    <div key={index} className="bg-gray-50 rounded-xl border border-gray-200 p-1.5 shadow-sm hover:shadow-md transition-shadow max-w-fit">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-1.5">
                          {attachment.type === 'file' ? (
                            <div className="w-5 h-5 bg-red-50 rounded flex items-center justify-center shadow-sm">
                              <FileText className="w-3 h-3 text-red-500" />
                            </div>
                          ) : (
                            <div className="w-5 h-5 bg-green-50 rounded flex items-center justify-center shadow-sm">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="text-xs text-gray-500">
                              {attachment.type === 'file' 
                                ? `PDF · ${formatFileSize((attachment.data as FileAttachment)?.size || 0)} · 约${(Math.round(((attachment.data as FileAttachment)?.content?.length || 0) / 1000) / 10).toFixed(1)}万字`
                                : '链接'
                              }
                            </div>
                          </div>
                        ...
                        <button
                          ...
                          className="p-0.5 hover:bg-gray-200 rounded-lg transition-colors ml-1"
                        >
                  ...
                  <div className="flex flex-wrap gap-1">
                    {getSuggestedActions(attachments).map((action, actionIndex) => (
                      <button
                        ...
                        className="bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-xl px-2 py-1 text-xs text-gray-700 flex items-center group transition-all duration-200 hover:shadow-sm whitespace-nowrap"
                      >
                        <span className="truncate">{action}</span>
                        <svg className="w-3 h-3 text-gray-400 group-hover:text-gray-600 transition-colors flex-shrink-0 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
