...
class SemanticSearchService:
    ...
    async def search_related_conversations(
        self,
        conversation_id: str,
        user_id: str,
        limit: int = 3
        ...
        try:
            ...
            # 搜索相关记忆
            related_memories = await self.search_semantic_memories(
            ...
            # 过滤掉当前对话
            filtered_results = [
                result for result in related_memories
    ...
    async def _get_conversation_summary(self, conversation_id: str) -> Optional[str]:
        ...
        try:
            # 从Redis缓存获取
            cached_conversation = await redis_manager.get_cached_conversation(conversation_id)
            if cached_conversation and cached_conversation.get("summary"):
                return cached_conversation["summary"]
    ...
    async def search_user_preferences(
        ...
        try:
            # 获取用户画像
            user_profile = await redis_manager.get_user_profile(user_id)
            
            # 构建偏好搜索查询
            preference_query = f"{query} 用户偏好"
            if user_profile.get("preferences"):
                preferences = ", ".join(user_profile["preferences"])
                preference_query += f" {preferences}"
            ...
            results = await self.search_semantic_memories(
                ...
                user_id=user_id,
                limit=limit,
                min_importance=0.3  # 偏好相关记忆重要性阈值较低
            )
            
            return results
            
        except Exception as e:
            app_logger.error(f"搜索用户偏好失败: {e}")
            return []
    ...
    async def search_by_intent(
        ...
        try:
            ...
            query = intent_queries.get(intent, intent)
    ...
    async def get_search_suggestions(
        ...
        try:
            # 获取用户最近的记忆
            recent_memories = await self.get_memory_timeline(user_id, days=7, limit=20)
